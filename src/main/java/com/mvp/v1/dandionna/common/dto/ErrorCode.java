 ...
 */
	...
	MENU_SET_COMPONENTS_REQUIRED(HttpStatus.BAD_REQUEST, "세트 메뉴는 최소 1개 이상의 구성품이 필요합니다."),
	MENU_SET_COMPONENT_INVALID(HttpStatus.BAD_REQUEST, "세트 메뉴 구성품이 올바르지 않습니다."),
	MENU_TYPE_IMMUTABLE(HttpStatus.BAD_REQUEST, "메뉴 유형은 변경할 수 없습니다."),
	MENU_COMPONENT_IN_USE(HttpStatus.CONFLICT, "다른 세트 메뉴에서 사용 중인 단품은 삭제할 수 없습니다."),
	MENU_NOT_ON_SALE(HttpStatus.CONFLICT, "판매중인 메뉴만 노쇼 게시가 가능합니다."),
	MENU_IMAGE_UPLOAD_INVALID(HttpStatus.BAD_REQUEST, "유효하지 않은 메뉴 이미지 업로드 토큰입니다."),
	MENU_IMAGE_UPLOAD_EXPIRED(HttpStatus.GONE, "만료된 메뉴 이미지 업로드 토큰입니다."),
	MENU_IMAGE_UPLOAD_NOT_CONFIRMED(HttpStatus.CONFLICT, "확인되지 않은 메뉴 이미지 업로드 토큰입니다."),
	MENU_IMAGE_UPLOAD_IN_USE(HttpStatus.CONFLICT, "이미 사용 중이거나 재사용할 수 없는 메뉴 이미지 업로드 토큰입니다."),
